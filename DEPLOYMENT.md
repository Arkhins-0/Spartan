...
Spartan is designed for easy deployment on Vercel with automatic database migrations, environment validation, and production-ready security configurations. This guide covers the complete deployment process from setup to monitoring.
...
3. Add each variable with appropriate values for each environment:
   - **Production**: `spartan.arkhins.com`, production Neon database, production Mailchimp sender, production `CRON_SECRET`
...
Preview deployments skip automatic migration deployment by default. Set `SPARTAN_RUN_MIGRATIONS_ON_BUILD=true` only for previews connected to a safe, isolated database.
...
Scheduled work lives in `app/api/cron/*` and is triggered by the GitHub Actions
workflow `.github/workflows/cron-jobs.yml`, not by Vercel Cron (Vercel Hobby
plans only allow once-daily schedules). The workflow curls each route with the
`CRON_SECRET` bearer token the routes already require:

| Route | Schedule (UTC) | Purpose |
|---|---|---|
| `/api/cron/rsvp-reminders` | hourly | RSVP reminders ~48 h before events |
| `/api/cron/notification-batches` | daily 08:00 | Batched notification digests |
| `/api/cron/event-waitlist` | every 10 min | Waitlist promotion backstop |
| `/api/cron/gear-notifications` | every 15 min | Gear outbox drain (ADR-0006) |
| `/api/cron/volunteer-reminders` | every 15 min | Volunteer reminders + outbox (ADR-0011) |

GitHub configuration required:

- **Secret `CRON_SECRET`** — the same value as the Vercel environment variable
  (`openssl rand -base64 48`; must be ≥ 32 characters). Without it every job
  fails fast with a clear error instead of hitting the site.
- **Variable `CRON_TARGET_URL`** (optional) — defaults to
  `https://spartan.arkhins.com`; set it to point the scheduler at a staging
  deployment.

Any job can be run on demand from the Actions tab via *Run workflow*. GitHub
may delay scheduled runs by a few minutes under load and pauses schedules on
repositories with no commits for 60 days; every route is idempotent and simply
catches up on its next run. If you move to a Vercel plan with unrestricted
...
docker run -p 3000:3000 --env-file .env.production spartan
```
...
3. **Process Management** (for VPS):
   ...
   # Using PM2
   bun add -g pm2
   pm2 start "bun run start" --name spartan
   pm2 startup
   pm2 save
   ```
...
This comprehensive deployment guide ensures a secure, performant, and maintainable production deployment of Spartan.
