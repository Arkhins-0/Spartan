...
**Why this priority**: USA Hockey requires member registration for all rostered participants — players and team officials alike. Having this field stored makes Spartan directly useful for USA Hockey affiliated organizations and makes the pitch credible.
...
**Why this priority**: A downloadable roster export is the closing argument in a hockey association pitch — it proves Spartan can replace manual spreadsheet tracking. Admin can hand this file directly to a registrar.
...
4. **Given** a CSV exported from Spartan, **When** opened in a spreadsheet application, **Then** special characters in names are correctly encoded (UTF-8) and the file is fully parseable.
...
- **SC-006**: A team registrar can capture all data fields required for USA Hockey affiliate paperwork using only Spartan, without resorting to a separate spreadsheet.
