# design-sync notes — Spartan

## What this repo is (read first)
Spartan is a **Next.js app, not a packaged component library** — no `dist/`, no
...
- **`--entry ./.design-sync/entry.ts` is required** — it's the aggregate entry that
  ...
  `window.Spartan`. Without it, PKG_DIR resolves to the non-existent
  `node_modules/spartan` and the build fails.
