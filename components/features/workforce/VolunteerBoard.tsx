  ...
  function handleCreate() {
    ...
    startTransition(async () => {
      const result = await createVolunteerNeed({
        ...
        timezone,
        signupMode: form.openSignup ? "OPEN_SIGNUP" : "INVITE_ONLY",
        ...(form.teamId ? { teamId: form.teamId } : {}),
        ...(form.postLabel ? { postLabel: form.postLabel } : {}),
      });
      if (result.success) {
        setCreateOpen(false);
        setForm({
          ...
          <Card key={need.id} variant="outlined" sx={{ p: 2 }}>
            ...
            >
              <Box>
                ...
                <Typography variant="body2" color="text.secondary">
                  {need.postLabel ? `${need.postLabel} · ` : ""}
                  {need.roundLabel ? `${need.roundLabel} · ` : ""}
                  {need.sessionLabel ? `${need.sessionLabel} · ` : ""}
                ...
                {need.briefingAt ? (
                  <Typography variant="caption" color="text.secondary">
                    Briefing{" "}
                    {new Date(need.briefingAt).toLocaleString(undefined, {
                      timeZone: need.timezone,
                    })}
                  </Typography>
                ) : null}
              ...
              <Stack direction="row" spacing={1} alignItems="center">
                ...
                {need.signupMode === "OPEN_SIGNUP" ? (
                  <Chip size="small" variant="outlined" label="Open signup" />
                ) : null}
              </Stack>
            </Stack>
            ...
            <Stack spacing={1} sx={{ mt: 2 }}>
                ...
                >
                  <Stack direction="row" spacing={1} alignItems="center">
                    ...
                    {assignment.checkedInAt ? (
                      <Chip size="small" color="success" label="Signed on" />
                    ) : null}
                  </Stack>
                  ...
                  <Stack direction="row" spacing={1}>
                    ...
                    (assignment.status === "ACCEPTED" ||
                      ...
                      <Button
                        ...
                        disabled={pending}
                        sx={{ minHeight: 44 }}
                        onClick={() => run(() => releaseVolunteerShift(assignment.id))}
                      >
                        {assignment.status === "WAITLISTED"
                      ...
                      <>
                        {!assignment.checkedInAt ? (
                          <Button
                            ...
                            disabled={pending}
                            sx={{ minHeight: 44 }}
                            onClick={() => run(() => checkInVolunteer(assignment.id))}
                          >
                            Sign on
                          </Button>
                        ) : null}
              ...
              {need.assignments.length === 0 ? (
                <Typography variant="body2" color="text.secondary">
                  {isOrganizer ? "Nobody assigned yet." : "This post is unclaimed."}
                </Typography>
              ) : null}

              {need.viewerWaitlistPosition !== null ? (
                <Typography variant="body2" color="warning.main">
                  You are number {need.viewerWaitlistPosition} on the waiting list.
                </Typography>
              ) : null}
              ...
              {need.canClaim ? (
                <Box>
                  <Button
                    ...
                    disabled={pending}
                    sx={{ minHeight: 44 }}
                    onClick={() => run(() => claimVolunteerShift(need.id))}
                  >
                    {need.acceptedCount >= need.capacity
      ...
      <Dialog open={createOpen} onClose={() => setCreateOpen(false)} fullWidth maxWidth="sm">
        ...
        <DialogContent>
          <Stack spacing={2} sx={{ mt: 1 }}>
            ...
            />
            <TextField
              label="Post or place (optional)"
              value={form.postLabel}
              onChange={(e) => setForm({ ...form, postLabel: e.target.value })}
              helperText="Where the shift is worked, e.g. &quot;Post 4 — Turn 3&quot;. Day sheets group by this."
              fullWidth
            />
            <FormControlLabel
              control={
                <Switch
                  ...
                  onChange={(e) => setForm({ ...form, openSignup: e.target.checked })}
                />
              }
              label="Let volunteers sign themselves up"
            />
            {teams.length > 0 ? (
              <TextField
                select
