 ...
 * manager, association admin), and delegation itself. Registration, finance,
 * and communications still guard on the legacy league/team roles, so offering
 * those here would hand out responsibilities that silently do nothing. They are
 * re-enabled as each action set is wired.
 *
 * Scheduler and event manager are offered because race rounds now check
 * MANAGE_SCHEDULE. Their guidance text says exactly how far that reaches, so an
 * administrator is not led to believe a scheduler grant also moves games.
 */
...
const SELECTABLE_SCOPES: AssociationRoleScopeType[] = [
  "ASSOCIATION",
  "DIVISION",
  "TEAM",
  "RACE_ROUND",
...
const ROLE_GUIDANCE: Record<AssociationRole, string> = {
  ...
  SCHEDULER:
    "Race rounds, results, and their timetables within the scope you choose. Track time requests, reservations, games, and practices still follow league admin membership.",
  ...
  EVENT_MANAGER:
    "One exact event or race weekend — its timetable, entries, and volunteers — without wider association access.",
  EQUIPMENT_MANAGER:
...
export interface RoleGrantManagerProps {
  ...
  /** Race weekends a responsibility can be bounded to. Empty for associations
   *  that do not run championships, which hides the scope option entirely. */
...
export function RoleGrantManager({
  ...
  // An association with no championship has nothing to bound a RACE_ROUND grant
  // to, so the option is dropped rather than offered as a dead end.
  const selectableScopes = useMemo(
    () =>
      SELECTABLE_SCOPES.filter(
        (scope) => scope !== "RACE_ROUND" || rounds.length > 0,
      ),
    [rounds.length],
  );
  ...
  function handleInvite() {
    ...
    startTransition(async () => {
      const result = await inviteAssociationOperator({
        ...
        ...(scopeType === "RACE_ROUND" ? { roundId: scopeId } : {}),
      });
  ...
  const needsScopeTarget =
    scopeType === "DIVISION" || scopeType === "TEAM" || scopeType === "RACE_ROUND";
