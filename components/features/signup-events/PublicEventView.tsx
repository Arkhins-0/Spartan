  ...
  return (
    <Stack spacing={2}>
      <PageHeader
        ...
        title={event.title}
        subtitle={`Hosted by ${hostName} · ${AGE_CLASSIFICATION_LABELS[event.ageClassification]}`}
        actions={
          event.status === "CANCELED" ? (
            <Chip color="error" label="Canceled" sx={{ alignSelf: "center" }} />
          ) : undefined
        }
      />
      ...
      <Card>
        <CardHeader
          title="Signup slots"
          subheader={`${event.slots.length} slot${event.slots.length === 1 ? "" : "s"}`}
        />
        <CardContent>
          <Stack spacing={2}>
      ...
      {myAssignments && myAssignments.length > 0 ? (
        <Card>
          <CardHeader title="Your team & games" />
          <CardContent>
            <Stack spacing={1.5}>
      ...
      {games && games.length > 0 ? (
        <Card>
          <CardHeader
            title="Game schedule"
            subheader={`${games.length} game${games.length === 1 ? "" : "s"}`}
          />
          <CardContent>
            <Stack spacing={1.5}>
              {games.map((game) => (
