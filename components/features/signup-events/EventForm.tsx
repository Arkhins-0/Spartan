...
const DEFAULT_SLOTS: SlotRow[] = [
  { name: "Driver", description: "", capacity: "40", priceDollars: "", waitlistEnabled: true },
...
export function EventForm({ hostOptions, venueOptions, initialValues, host }: EventFormProps) {
  ...
  const handleSubmit = (event: FormEvent<HTMLFormElement>) => {
    ...
    if (slotInputs.length === 0) {
      setError("Add at least one signup slot (e.g. Driver ×40).");
      return;
    }
          ...
          <Stack spacing={2}>
            ...
            {!isEdit ? (
              <TextField
                ...
                helperText="The circuit, championship, or team hosting this event"
              >
                {hostOptions.map((option) => (
                  <MenuItem key={`${option.kind}:${option.id}`} value={`${option.kind}:${option.id}`}>
                    {option.name}
                    {option.kind === "organization" ? " (circuit)" : option.kind === "league" ? " (championship)" : " (team)"}
                  </MenuItem>
                ))}
              </TextField>
            ) : null}
            ...
            <Stack direction={{ xs: "column", sm: "row" }} spacing={2}>
              ...
              <TextField
                ...
                placeholder="Main paddock — enter through gate B"
        ...
        <CardContent>
          <Stack spacing={2}>
            ...
            <Typography variant="body2" color="text.secondary">
              Each slot has its own capacity — e.g. Driver ×30, Marshal ×40, Scrutineer ×4, Official ×8.
            ...
            {acceptsManual ? (
              <Stack direction={{ xs: "column", sm: "row" }} spacing={2} flexWrap="wrap" useFlexGap>
                ...
                <TextField
                  ...
                  placeholder="Cash accepted at the gate. Include your driver's name in the payment note."
