...
interface SegmentationEditorProps {
  surfaceId: string;
  /** Effective whole-surface label ("Full circuit", custom rename, …). */
  wholeLabel: string;
...
export function SegmentationEditor({
  ...
  // Colour roles read through theme.vars (CSS custom properties) so they
  // follow the colour scheme — theme.palette.X.main would return the LIGHT
  // literal under CSS variables. The test harness renders without variables,
  // hence the fallback to the static palette. Applied via `style` because SVG
  // presentation attributes do not reliably resolve var().
  ...
  const kindColor = (kind: string): string => {
    switch (kind) {
      ...
      default:
        // Custom zones: the theme's `secondary` is a receding control fill
        // (near-white by day), which would vanish on the card — use the muted
        // ink instead so a drawn zone stays legible in both schemes.
        ...
        >
          ...
          >
            ...
            <rect
              ...
              rx={12}
              strokeWidth={1}
              style={{ fill: palette.action.hover, stroke: palette.divider }}
            />
            <text
              x={VIEW_W / 2}
              y={VIEW_H - 5}
            ...
            {draftRect ? (
              <rect
                ...
                strokeDasharray="3 2"
                pointerEvents="none"
                style={{ fill: palette.text.secondary, stroke: palette.text.secondary }}
              />
            ) : null}
            ...
            return (
              ...
              >
                ...
                <Stack direction="row" spacing={1}>
                  <Button variant="outlined" onClick={() => openEdit(segment)}>
                    Edit
                  </Button>
                  <Button
                    variant="outlined"
