...
export function PublicRinkProfileCard({ venue }: { venue: PublicVenueSummary }) {
  ...
  return (
    <Card sx={{ height: "100%" }}>
      {venue.slug ? (
        <LinkCardActionArea href={`/rinks/${venue.slug}`} sx={{ height: "100%", alignItems: "stretch" }}>
          {body}
        </LinkCardActionArea>
      ) : (
        body
  ...
  return (
    <Stack spacing={2}>
      <PageHeader
        ...
        title={venue.name}
        subtitle={location || "Public rink profile"}
        actions={
          venue.slug ? (
            <LinkButton href={`/rinks/${venue.slug}/schedule`} variant="contained">
              View full schedule
            </LinkButton>
          ) : undefined
        }
      ...
      <Card>
        <CardHeader
          ...
          action={
            venue.slug ? (
              <LinkButton href={`/rinks/${venue.slug}/schedule`} variant="outlined" size="small">
                View full schedule
              </LinkButton>
            ) : undefined
          }
