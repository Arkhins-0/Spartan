 ...
 * US1 / T013), replacing the read-only VenueSurfaceManager on the venue-admin
 * surfaces page.
 *
 ...
 *   whole-surface label) via createVenueSurface / updateVenueSurface /
          ...
          <Stack spacing={2}>
            ...
            {surfaces.length === 0 ? (
              <Typography variant="body2" color="text.secondary">
                No surfaces yet — add your first circuit, track, paddock, or room.
              </Typography>
            ) : (
              <Box>
  ...
  const handleSubmit = (event: FormEvent<HTMLFormElement>) => {
    ...
    void (async () => {
      ...
      const result = surface
        ? await updateVenueSurface({ ...base, surfaceId: surface.id })
        : await createVenueSurface(base);
      if (!result.success) {
        setSaving(false);
