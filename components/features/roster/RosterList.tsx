...
type RosterListProps = {
  players: Player[];
  /**
   * Drives sport-specific roster fields. Defaults to the generic path so
   * existing callers that never pass it keep today's behavior.
   */
  sport?: Sport;
    ...
    <>
      ...
      {isAdmin && (
        <Box sx={{ mb: 3, display: "flex", gap: 2, flexWrap: "wrap" }}>
          ...
          >
            {isMotorsport ? "Add Driver" : "Add Player"}
          </Button>
          <Button
            variant="outlined"
      ...
      {players.length === 0 ? (
        <EmptyState
          ...
          description={
            isAdmin
              ? `Get started by adding your first ${isMotorsport ? "driver" : "player"}`
              : `Contact your team admin to add ${isMotorsport ? "drivers" : "players"}`
          }
        />
      ) : (
