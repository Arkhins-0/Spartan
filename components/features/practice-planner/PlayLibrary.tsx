...
export interface PlayLibraryProps {
    ...
    mode?: "select" | "manage";
    /**
     * Render the library's own "Play Library" heading and New Play action.
     * Pass false when the host already provides a title (the library page's
     * PageHeader, or the session editor's dialog title) so there is one
     * title per screen.
     */
    showHeader?: boolean;
}
    ...
    return (
        <Card
            sx={(theme) => ({
                ...
                // Selection is the one accent on the card: a 2px primary
                // outline read through theme.vars so it follows the scheme.
                outline: isSelected
        ...
        >
            <CardMedia
                ...
                sx={{
                    height: 180,
                    bgcolor: "action.hover",
                    borderBottom: "1px dashed var(--sp-border-input)",
                    display: "flex",
                    alignItems: "center",
                    justifyContent: "center",
                    position: "relative",
...
export function PlayLibrary({
    ...
    const isFiltered = Boolean(searchQuery) || dateFilter !== "all";

    return (
        <Box>
            {/* Header (only when the host does not already title the screen) */}
            {showHeader && (
                <Stack
                    direction="row"
            ...
            {!isLoading && plays.length === 0 && (
                <EmptyState
                    icon={isFiltered ? <SearchIcon /> : <LibraryIcon />}
                    title={isFiltered ? "No plays found" : "No plays in your library yet"}
                    description={
                        isFiltered
            ...
            >
                ...
                <DialogActions>
                    <Button onClick={handleDeleteCancel} disabled={isDeleting} variant="text">
                        Cancel
                    </Button>
                    <Button
