    ...
    <PageContainer maxWidth="md">
      ...
      <Stack spacing={3}>
        <Typography variant="h5" component="h2">
          Circuit sessions &amp; programmes
        </Typography>
        {registrations.length === 0 ? (
          <EmptyState
            icon={<ReceiptLongIcon />}
            title="No session registrations yet"
            description="Browse a circuit's schedule to register for public sessions and programmes."
            action={
              <LinkButton href="/circuits" variant="outlined">
                Browse circuits
              </LinkButton>
            }
          />
                  ...
                  <CardContent>
                    <Stack spacing={1}>
                      ...
                      <Typography variant="body2" color="text.secondary">
                        {reg.venue.slug ? (
                          <Link href={`/circuits/${reg.venue.slug}`}>{reg.venue.name}</Link>
                        ) : (
                          reg.venue.name
                        )}
