...
export default async function VenueSchedulePage({ params }: VenueSchedulePageProps) {
  ...
  // the board's days line up with the circuit's wall clock. The client owns week
  ...
  return (
    <PageContainer>
      ...
      <Stack spacing={3}>
        <VenueSurfaceManager organizationId={organizationId} venueId={venueId} surfaces={surfaces} />
        <OperatingHoursEditor organizationId={organizationId} venueId={venueId} operatingHours={operatingHours} />
        <AvailableSurfaceBrowser
          blocks={result.data.availableSurface}
          timeZone={timeZone}
          mode="staff"
        />
        <VenueScheduleBoard
