...
export default async function VenueReservationsPage({
  ...
  const rawReservations = await prisma.venueReservation.findMany({
      where: {
        status: "CONFIRMED",
        OR: [
  ...
  return (
    <PageContainer maxWidth="xl">
      <PageHeader
        ...
        subtitle={`Confirmed track-time inventory for ${membership.league.name}. Times are shown in each venue's local timezone.`}
      />
      <VenueReservationInventory
        leagueId={leagueId}
        reservations={items}
