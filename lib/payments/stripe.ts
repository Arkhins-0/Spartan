...
export function getStripeClient(): Stripe {
  ...
  if (!globalForStripe.stripe) {
    globalForStripe.stripe = new Stripe(env.STRIPE_SECRET_KEY, {
      appInfo: { name: "Spartan", url: "https://spartan.arkhins.com" },
      typescript: true,
    });
  }
...
export function computeApplicationFee(amountCents: number, feeBps?: number | null): number {
  ...
  // Round down so the platform never over-charges the circuit.
...
export async function createConnectAccount(input: ConnectAccountInput): Promise<Stripe.Account> {
  ...
  return stripe.accounts.create({
    ...
    business_profile: {
      ...
      product_description: "Circuit sessions, coaching, and programmes",
    },
    capabilities: {
...
/**
 * Create an onboarding/account-management link the circuit owner uses to complete
...
/**
 * Create a Stripe Express dashboard login link so onboarded circuits can view payouts.
 */
...
/**
 * Create a Checkout Session as a DIRECT charge on the circuit's connected account.
 * The customer pays the circuit; Spartan collects an optional application fee.
 */
...
export async function refundPaymentIntent(input: {
  ...
  connectedAccountId: string;
  amount?: number;
  /** Reverse the application fee back to the circuit (default true for full refunds). */
