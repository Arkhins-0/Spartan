 ...
 * drafts and canceled games hold no track time), EventGames (parent SignupEvent
...
async function findBookingConflictsFromSources(
  ...
  const [reservations, events, seasonGames, eventGames, scheduleBlocks, coexistenceKeys] =
    await Promise.all([
      includeReservations
  ...
  return [...reservations, ...events, ...seasonGames, ...eventGames, ...scheduleBlocks]
...
/**
 * All bookings at a venue over [from, to) from the same sources with the
...
export async function getVenueBookings(params: {
  ...
  const [reservations, events, seasonGames, eventGames, scheduleBlocks] = await Promise.all([
  ...
  return [...reservations, ...events, ...seasonGames, ...eventGames, ...scheduleBlocks].sort(
    byStartAt
  );
}
...
// already clipped to the window by the query (plus JS for expanded block
// occurrences).
// ---------------------------------------------------------------------------
