...
// Spartan is a motorsport-only platform: MOTORSPORT is the sole value.
export const SPORTS = ["MOTORSPORT"] as const;
...
export const FEATURED_SPORTS: SportValue[] = ["MOTORSPORT"];

export const sportSchema = z.enum(SPORTS, {
  message: "Please select a valid sport",
});

export const SURFACE_TYPES = ["CIRCUIT", "TRACK", "PADDOCK", "ROOM", "OTHER"] as const;
export const VENUE_ORGANIZATION_TYPES = ["CIRCUIT", "KART_TRACK", "DRAG_STRIP", "MOTORSPORT_COMPLEX", "OTHER"] as const;
...
export const SKILL_LEVEL_SOURCES = ["FMSCI", "CLUB_CUSTOM", "OTHER"] as const;
export const SKILL_LEVEL_DISCIPLINES = ["CIRCUIT_RACING", "KARTING", "RALLY", "DRAG_RACING", "HILL_CLIMB", "OTHER"] as const;
 ...
 * the raw enum (e.g. "MOTORSPORT") directly.
 */
...
export const addPlayerSchema = z.object({
  ...
  // Motorsport roster fields.
  fmsciLicenseNumber: z
    .string()
    .trim()
    .max(30, "FMSCI licence number must be 30 characters or fewer")
    .regex(/^[a-zA-Z0-9/-]*$/, "FMSCI licence number must be alphanumeric (hyphens and slashes allowed)")
    .nullable()
    .optional(),
  // Car numbers are not bounded 1–99 the way jersey numbers are; national
  // championship entries routinely run three digits.
  carNumber: z
    .number()
    .int("Car number must be a whole number")
    .min(0, "Car number must be between 0 and 999")
    .max(999, "Car number must be between 0 and 999")
    .nullable()
    .optional(),
  racingClass: optionalSanitizedString(50),
...
// ---------------------------------------------------------------------------
// Operational instruction/request threads (two-way, reply-tracked).
// ---------------------------------------------------------------------------

// An association admin issues an instruction to one, some, or all teams.
// Targeting mirrors sendLeagueMessageSchema so the two composers agree, but it
// resolves to teams rather than individual users.
export const createInstructionSchema = z.object({
  leagueId: z.string().cuid("Invalid league ID format"),
  subject: sanitizedStringWithMin(1, 200),
  body: sanitizedStringWithMin(1, 5000),
  priority: z.enum(["LOW", "NORMAL", "HIGH", "URGENT"]).default("NORMAL"),
  // False makes the instruction informational — targets start ACKNOWLEDGED
  // and the admin is not shown an outstanding-response count.
...
// A team raises a request or update with the association.
...
// A reply from either side. actorTeamId identifies the replying team; omit it
// when the association is replying.
export const postThreadEntrySchema = z.object({
  threadId: z.string().cuid("Invalid thread ID format"),
  body: sanitizedStringWithMin(1, 5000),
  actorTeamId: optionalCuid("Invalid team ID format"),
});
export type PostThreadEntryInput = z.infer<typeof postThreadEntrySchema>;
...
export const getLeagueThreadsSchema = z.object({
  leagueId: z.string().cuid("Invalid league ID format"),
  kind: z.enum(["INSTRUCTION", "TEAM_REQUEST"]).optional(),
  status: z.enum(["OPEN", "RESOLVED", "CLOSED"]).optional(),
});
export type GetLeagueThreadsInput = z.infer<typeof getLeagueThreadsSchema>;

export const getTeamThreadsSchema = z.object({
  teamId: z.string().cuid("Invalid team ID format"),
});
export type GetTeamThreadsInput = z.infer<typeof getTeamThreadsSchema>;

// ---------------------------------------------------------------------------
// Championship rounds and results.
// ---------------------------------------------------------------------------

const raceRoundBaseSchema = z.object({
  name: sanitizedStringWithMin(1, 120),
  roundNumber: z
    .number()
    .int("Round number must be a whole number")
    .min(1, "Round number must be at least 1")
    .max(99, "Round number must be 99 or lower"),
  // Date-only (YYYY-MM-DD); the action converts to UTC midnight.
  raceDate: z
    .string()
    .trim()
  ...
  locationText: optionalSanitizedString(200),
  notes: optionalSanitizedString(1000),
});

export const createRaceRoundSchema = raceRoundBaseSchema.extend({
  leagueId: z.string().cuid("Invalid league ID format"),
});
export type CreateRaceRoundInput = z.infer<typeof createRaceRoundSchema>;

export const updateRaceRoundSchema = raceRoundBaseSchema.extend({
  roundId: z.string().cuid("Invalid round ID format"),
  status: z.enum(["SCHEDULED", "RESULTS_PENDING", "FINALIZED"]).optional(),
});
export type UpdateRaceRoundInput = z.infer<typeof updateRaceRoundSchema>;

// One entrant's line in the results grid. Position is required for a
// CLASSIFIED finish and must be absent otherwise — a car that did not finish
// has no finishing position, and storing one would corrupt the standings.
export const raceResultEntrySchema = z
  .object({
    teamId: z.string().cuid("Invalid team ID format"),
    playerId: optionalCuid("Invalid driver ID format"),
    position: z
      .number()
      .int("Position must be a whole number")
      .min(1, "Position must be at least 1")
      .max(999, "Position must be 999 or lower")
      .nullable()
      .optional(),
    points: z
      .number()
      .min(0, "Points cannot be negative")
      .max(9999, "Points must be 9999 or lower")
      .default(0),
    status: z.enum(["CLASSIFIED", "DNF", "DNS", "DSQ"]).default("CLASSIFIED"),
    notes: optionalSanitizedString(500),
  })
  .refine((data) => data.status !== "CLASSIFIED" || data.position != null, {
    message: "A classified finish needs a position",
    path: ["position"],
  })
  .refine((data) => data.status === "CLASSIFIED" || data.position == null, {
    message: "Only a classified finish can have a position",
    path: ["position"],
  });

export const recordRaceResultsSchema = z.object({
  roundId: z.string().cuid("Invalid round ID format"),
  results: z
    .array(raceResultEntrySchema)
    .min(1, "Record at least one result")
    .max(200, "A round cannot hold more than 200 entries"),
  // Leave the round open for corrections instead of finalizing it.
  finalize: z.boolean().default(true),
});
export type RecordRaceResultsInput = z.infer<typeof recordRaceResultsSchema>;

export const getChampionshipStandingsSchema = z.object({
  leagueId: z.string().cuid("Invalid league ID format"),
  seasonId: optionalCuid("Invalid season ID format"),
  // Team standings aggregate every entry; driver standings split them out.
  groupBy: z.enum(["TEAM", "DRIVER"]).default("TEAM"),
});
export type GetChampionshipStandingsInput = z.infer<typeof getChampionshipStandingsSchema>;
...
// ---------------------------------------------------------------------------
// Race weekend timetable and entry list.
// ---------------------------------------------------------------------------

export const RACE_SESSION_KINDS = [
  "PRACTICE",
  "QUALIFYING",
  "RACE",
  "SUPPORT",
...
export const RACE_SESSION_KIND_LABELS: Record<
  (typeof RACE_SESSION_KINDS)[number],
  string
> = {
  PRACTICE: "Practice",
  ...
  ADMIN: "Sign-on, scrutineering, or briefing",
};

const raceSessionBaseSchema = z
  .object({
    name: sanitizedStringWithMin(1, 120),
    kind: z.enum(RACE_SESSION_KINDS).default("RACE"),
    // Absolute instants. The venue-local reading comes from `timezone`, which
    // is authoritative for display per FR-012.
    startAt: z.coerce.date(),
    endAt: z.coerce.date(),
    timezone: z.string().min(1, "A timezone is required").max(64),
    sortOrder: z
      .number()
  ...
  // A segment is part of a surface, so naming one without the other leaves the
  // conflict check unable to say what space is claimed.
  .refine((value) => !value.segmentId || Boolean(value.surfaceId), {
    message: "Select the surface the segment belongs to",
    path: ["surfaceId"],
  })
  .refine((value) => !value.surfaceId || Boolean(value.venueId), {
    message: "Select the venue the surface belongs to",
    path: ["venueId"],
  });

export const createRaceSessionSchema = z.intersection(
  raceSessionBaseSchema,
  z.object({ roundId: z.string().cuid("Invalid round ID format") }),
);
export type CreateRaceSessionInput = z.input<typeof createRaceSessionSchema>;

export const updateRaceSessionSchema = z.intersection(
  raceSessionBaseSchema,
  z.object({ sessionId: z.string().cuid("Invalid session ID format") }),
);
export type UpdateRaceSessionInput = z.input<typeof updateRaceSessionSchema>;
...
export const RACE_ENTRY_STATUSES = ["PROVISIONAL", "CONFIRMED", "WITHDRAWN"] as const;

export const VOLUNTEER_CREDENTIAL_KINDS = [
  "MARSHAL_GRADE",
  "LICENCE",
  "FIRST_AID",
  "SCRUTINEER",
...
export const VOLUNTEER_CREDENTIAL_KIND_LABELS: Record<
  (typeof VOLUNTEER_CREDENTIAL_KINDS)[number],
  string
> = {
  MARSHAL_GRADE: "Marshal grade",
  LICENCE: "Licence",
  FIRST_AID: "First aid",
...
export const upsertRaceEntrySchema = z.object({
  roundId: z.string().cuid("Invalid round ID format"),
  teamId: z.string().cuid("Invalid team ID format"),
  playerId: optionalCuid("Invalid driver ID format"),
  // Free text, not a number: racing numbers are commonly "07" or "1A", and the
  // leading zero is part of the identity.
  carNumber: optionalSanitizedString(10),
  className: optionalSanitizedString(60),
  status: z.enum(RACE_ENTRY_STATUSES).default("PROVISIONAL"),
  notes: optionalSanitizedString(500),
});
export type UpsertRaceEntryInput = z.input<typeof upsertRaceEntrySchema>;

export const raceEntryIdSchema = z.object({
  entryId: z.string().cuid("Invalid entry ID format"),
});
export type RaceEntryIdInput = z.infer<typeof raceEntryIdSchema>;
...
export const DOCUMENT_KIND_LABELS: Record<(typeof DOCUMENT_KINDS)[number], string> = {
  ENTRY_FORM: "Entry form",
  TECH_INSPECTION: "Scrutineering / tech inspection",
  MEDICAL_CERTIFICATE: "Medical certificate",
  RESULTS_SHEET: "Results sheet",
  OTHER: "Other",
};

// Recorded after the client's direct-to-storage upload resolves; the action
// re-checks the storage reference, content type, and size server-side.
export const finalizeDocumentSchema = z.object({
  leagueId: z.string().cuid("Invalid league ID format"),
  teamId: optionalCuid("Invalid team ID format"),
  threadId: optionalCuid("Invalid thread ID format"),
  kind: z.enum(DOCUMENT_KINDS).default("OTHER"),
  title: sanitizedStringWithMin(1, 200),
  // Object key (S3) or blob URL (Vercel Blob) — see lib/storage.
  key: z.string().min(1, "Missing upload reference").max(1000),
  contentType: z.string().min(1).max(100),
  sizeBytes: z.number().int().min(1, "File appears to be empty"),
});
...
export const createVenueOrganizationSchema = z.object({
  name: sanitizedStringWithMin(1, 100),
  type: z.enum(VENUE_ORGANIZATION_TYPES).default("CIRCUIT"),
...
export const createVenueSurfaceSchema = z.object({
  organizationId: z.string().cuid("Invalid organization ID format"),
  venueId: z.string().cuid("Invalid venue ID format"),
  name: sanitizedStringWithMin(1, 100),
  surfaceType: z.enum(SURFACE_TYPES).default("CIRCUIT"),
  capacity: optionalPositiveInt("Capacity must be a positive whole number"),
...
export const updateVenueSurfaceSchema = createVenueSurfaceSchema.extend({
...
export const submitSurfaceTimeRequestSchema = z
  .object({
...
export const decideSurfaceTimeRequestSchema = z.object({
  organizationId: z.string().cuid("Invalid organization ID format"),
  venueId: z.string().cuid("Invalid venue ID format"),
...
// --- Circuit session registration & purchasing ---
...
export type CreateVenueSurfaceInput = z.input<typeof createVenueSurfaceSchema>;
export type UpdateVenueSurfaceInput = z.input<typeof updateVenueSurfaceSchema>;
...
export type SubmitSurfaceTimeRequestInput = z.input<typeof submitSurfaceTimeRequestSchema>;
export type DecideSurfaceTimeRequestInput = z.input<typeof decideSurfaceTimeRequestSchema>;
