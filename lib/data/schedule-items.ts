...
async function readPractices(
  ...
  const rows = await findMany<{
    ...
    select: {
      ...
      duration: true,
      startAt: true,
      // PracticeSession carries no timezone column of its own; the item's
      // zone resolves from its reservation or venue below (a `timezone: true`
      // here made every /calendar and dashboard load throw
...
export function buildScheduleIcs(
  ...
  const lines = [
    "BEGIN:VCALENDAR",
    "VERSION:2.0",
    `PRODID:${options.prodId ?? "-//Spartan//Schedule//EN"}`,
    "CALSCALE:GREGORIAN",
    "METHOD:PUBLISH",
  ...
  for (const item of items) {
    ...
    lines.push(
      "BEGIN:VEVENT",
      `UID:${escapeIcsText(item.canonicalScheduleId)}@spartan.arkhins.com`,
