...
export type AssociationOperationsData = {
  ...
  counts: {
    pendingTrackTimeRequests: number;
    ...
    unstaffedRounds: number;
    urgentGearNeeds: number;
    overdueGearCustody: number;
    outboxPending: number;
    outboxFailed: number;
  };
  pendingTrackTimeRequests: OperationsAction[];
  ...
  unstaffedRounds: OperationsAction[];
  gear: {
...
export async function getAssociationOperationsData(
  ...
  const [
    ...
    prisma.surfaceTimeRequest.findMany({
      where: {
    ...
    // A race weekend with no volunteer needs at all is a different problem
    // from a need that is short: nobody has started staffing it. That is the
    // one an organizer discovers too late.
    prisma.raceRound.findMany({
      ...
      select: { id: true, name: true, roundNumber: true, raceDate: true },
  ...
  const volunteerShortages = volunteerNeeds.map((need) => ({
      id: need.id,
      title: need.postLabel ? `${need.roleLabel} — ${need.postLabel}` : need.roleLabel,
  ...
  const unstaffedRounds = upcomingRounds.map((round) => ({
    id: round.id,
    title: `Round ${round.roundNumber} — ${round.name}`,
    detail: "No volunteer posts created yet",
    href: href(leagueId, `/rounds/${round.id}`),
    at: round.raceDate.toISOString(),
  }));
  ...
  return {
    ...
    counts: {
      pendingTrackTimeRequests: requests.length,
      ...
      unstaffedRounds: unstaffedRounds.length,
    ...
    pendingTrackTimeRequests: requests.map((request) => ({
      id: request.id,
      title: `Track time request${request.requesterTeam?.name ? ` · ${request.requesterTeam.name}` : ""}`,
