...
interface SurfaceTimeRequestSubmittedEmailData {
...
export async function sendVenueRelationshipInvitationEmail(
  ...
  const relationshipLabel = data.relationshipType === "HOME" ? "home circuit" : "preferred circuit";
...
export async function sendSurfaceTimeRequestSubmittedEmail(
  data: SurfaceTimeRequestSubmittedEmailData
): Promise<void> {
  ...
  await sendEmail({
    subject: `New track time request for ${data.venueName}`,
...
interface SurfaceTimeRequestDecisionEmailData {
  contactEmail: string;
  venueName: string;
...
export async function sendSurfaceTimeRequestDecisionEmail(
  data: SurfaceTimeRequestDecisionEmailData
): Promise<void> {
  const statusLabel =
  ...
  await sendEmail({
    subject: `Your track time request was ${statusLabel}`,
    html: `<p>${escapeHtml(data.venueName)} ${statusLabel} your track time request.</p>${data.decisionMessage ? `<p>${escapeHtml(data.decisionMessage)}</p>` : ""}`,
    text: `${data.venueName} ${statusLabel} your track time request.${data.decisionMessage ? `\n\n${data.decisionMessage}` : ""}`,
...
/**
 * Notify circuit managers that a new registration was received.
 */
  ...
  const message: EmailMessage = {
    ...
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        ...
        <p>Spartan is a free platform for managing motorsport teams and championships. You'll be able to:</p>
        <ul>
          <li>View the team entry list</li>
          <li>See upcoming race weekends and sessions</li>
...
Spartan is a free platform for managing motorsport teams and championships. You'll be able to view the team entry list, see upcoming race weekends and sessions, RSVP to events, and stay connected with your team.
  ...
  const message: EmailMessage = {
    ...
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        ...
        <p>You can now view the team entry list, see upcoming race weekends and sessions, and RSVP to events.</p>
...
You can now view the team entry list, see upcoming race weekends and sessions, and RSVP to events.
...
export async function sendEventCreatedEmail(data: EventCreatedEmailData): Promise<void> {
  ...
  const eventTypeLabel = data.eventType === "GAME" ? "Race" : "Practice Session";
...
export async function sendEventTeamsUpdateEmail(data: EventTeamsUpdateEmailData): Promise<void> {
  ...
  for (const recipient of data.recipients) {
    try {
      await sendEmail({
        subject: headline,
        html: `<p>${escapeHtml(headline)}.</p><p><a href="${eventLink}">See your team and session times</a></p>`,
        text: `${headline}. See your team and session times: ${eventLink}`,
        to: [{ email: recipient.email }]
  });
    } catch (emailError) {
...
export async function sendGameProposalNotifications(
  ...
  const subjects: Record<GameProposalChange, string> = {
    created: `New race proposal from ${actingTeam.name}`,
    countered: `${actingTeam.name} countered your race proposal`,
    accepted: `${actingTeam.name} accepted your race proposal`,
    declined: `${actingTeam.name} declined your race proposal`,
    withdrawn: `${actingTeam.name} withdrew the race proposal`,
  };
  const headlines: Record<GameProposalChange, string> = {
    created: "New Race Proposal",
    countered: "Race Proposal Countered",
    accepted: "Race Proposal Accepted",
    declined: "Race Proposal Declined",
    withdrawn: "Race Proposal Withdrawn",
  };
  const leads: Record<GameProposalChange, string> = {
    created: `${actingTeam.name} has proposed a race with ${recipientTeam.name}.`,
    countered: `${actingTeam.name} has counter-proposed new terms for your race proposal.`,
    accepted: `${actingTeam.name} has accepted the race proposal. The race has been added to the schedule.`,
    declined: `${actingTeam.name} has declined the race proposal.`,
    withdrawn: `${actingTeam.name} has withdrawn the race proposal.`,
  };
