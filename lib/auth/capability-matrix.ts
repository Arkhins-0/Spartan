...
export enum Capability {
  ...
  /** Venue reservations and track time requests. */
...
export const ROLE_CAPABILITY_MATRIX: Record<AssociationRole, RoleEntry> = {
  ...
  SCHEDULER: {
    ...
    scopes: ["ASSOCIATION", "DIVISION", "TEAM", "SEASON", "RACE_ROUND"],
  },
  REGISTRAR: {
  ...
  VOLUNTEER_COORDINATOR: {
    ...
    scopes: ["ASSOCIATION", "DIVISION", "TEAM", "SEASON", "EVENT", "RACE_ROUND"],
  },
  // A race weekend is run by one person on the ground. MANAGE_SCHEDULE is here
  // so that person can move a session without holding association-wide
  // scheduling authority; the RACE_ROUND scope is what keeps it to their round.
  EVENT_MANAGER: {
    capabilities: [
    ...
    scopes: ["EVENT", "SIGNUP_EVENT", "RACE_ROUND"],
  },
  EQUIPMENT_MANAGER: {
