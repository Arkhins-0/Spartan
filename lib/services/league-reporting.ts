...
interface FinancialSurfaceTimeRequest {
    status: string;
    scheduleBlock: {
...
function summarizeSurfaceTimeRequests(requests: FinancialSurfaceTimeRequest[]) {
    ...
    const rows = players.map(player => {
        ...
        if (includeAdminFields) {
            row.push(
                ...
                'fmsciLicenseNumber' in player ? player.fmsciLicenseNumber || '' : ''
            );
        }
 ...
 *
 * Spartan does not currently maintain a payment ledger, so this report uses
 * real league data that exists today: roster size, event volume, track-time
...
export async function generateFinancialReportCSV(
    ...
    const rows = league.teams.map((team) => {
        const summary = summarizeSurfaceTimeRequests(team.surfaceTimeRequests);
        ...
        return [
            ...
            summary.pricedApprovedRequests > 0
                ? 'Accepted priced track-time requests only'
                : 'No accepted priced track-time requests',
        ];
    });

    const leagueWideSummary = summarizeSurfaceTimeRequests(league.surfaceTimeRequests);
    if (leagueWideSummary.totalRequests > 0) {
        rows.push([
            ...
            leagueWideSummary.pricedApprovedRequests > 0
                ? 'Accepted priced league-level track-time requests only'
                : 'No accepted priced league-level track-time requests',
        ]);
    }
    ...
    const allRequests = [
        ...league.surfaceTimeRequests,
        ...league.teams.flatMap((team) => team.surfaceTimeRequests),
    ];
    const totalSummary = summarizeSurfaceTimeRequests(allRequests);
    ...
    rows.push([
        ...
        'Known costs sum accepted track-time requests with recorded venue prices',
    ]);
