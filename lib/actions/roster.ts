...
/**
 * Warn-only duplicate checks for the roster's competitor numbers.
 *
 * Neither jerseyNumber nor carNumber carries a unique constraint — a squad can
 * legitimately hold a duplicate mid-season while paperwork catches up — so a
 * collision surfaces as a warning instead of blocking the write.
 */
async function findDuplicateNumberWarning(
  teamId: string,
  excludePlayerId: string,
  numbers: { jerseyNumber?: number | null; carNumber?: number | null }
): Promise<string | undefined> {
  const warnings: string[] = [];

  if (numbers.jerseyNumber != null) {
    const duplicate = await prisma.player.findFirst({
      where: { teamId, jerseyNumber: numbers.jerseyNumber, id: { not: excludePlayerId } },
      select: { name: true },
    });
    if (duplicate) {
      warnings.push(`Jersey #${numbers.jerseyNumber} is already assigned to ${duplicate.name}`);
    }
  }

  if (numbers.carNumber != null) {
    const duplicate = await prisma.player.findFirst({
      where: { teamId, carNumber: numbers.carNumber, id: { not: excludePlayerId } },
      select: { name: true },
    });
    if (duplicate) {
      warnings.push(`Car #${numbers.carNumber} is already assigned to ${duplicate.name}`);
    }
  }
...
export async function addPlayer(input: AddPlayerInput) {
  try {
    ...
    const player = await prisma.$transaction(async (tx) => {
      const created = await tx.player.create({
        data: {
          ...
          fmsciLicenseNumber: validated.fmsciLicenseNumber || null,
          carNumber: validated.carNumber ?? null,
          racingClass: validated.racingClass || null,
          dateOfBirth,
        },
      });
    ...
    // Check for duplicate competitor numbers on the same team
    const warning = await findDuplicateNumberWarning(validated.teamId, player.id, {
      jerseyNumber: validated.jerseyNumber,
      carNumber: validated.carNumber,
    });
...
export async function updatePlayer(input: UpdatePlayerInput) {
  try {
    ...
    const player = await prisma.$transaction(async (tx) => {
      const updated = await tx.player.update({
        ...
        data: {
          ...
          ...(validated.fmsciLicenseNumber !== undefined && {
            fmsciLicenseNumber: validated.fmsciLicenseNumber || null,
          }),
          ...(validated.carNumber !== undefined && { carNumber: validated.carNumber }),
          ...(validated.racingClass !== undefined && { racingClass: validated.racingClass || null }),
    ...
    // Check for duplicate competitor numbers on the same team (excluding the updated player)
    const warning = await findDuplicateNumberWarning(validated.teamId, validated.id, {
      jerseyNumber: validated.jerseyNumber,
      carNumber: validated.carNumber,
    });
