...
export async function getLeagueRosterData(leagueId: string): Promise<{
  ...
  players: Array<{
    ...
    fmsciLicenseNumber: string | null;
  ...
  const [players, teams, divisions, rawInvitations] = await Promise.all([
    prisma.player.findMany({
      ...
      // which leaked emergency contacts + licence numbers to any league MEMBER.
      ...
      select: {
        ...
        jerseyNumber: true,
        position: true,
        // The FMSCI licence is a governing-body identifier, so it is
        // admin-only (FR-008). Car number and class are roster
        // identity the whole league sees, like jerseyNumber and position.
        fmsciLicenseNumber: isLeagueAdmin,
        carNumber: true,
        racingClass: true,
