...
export async function signup(data: SignupWithInvitationInput) {
  try {
    ...
    // Outside production, no real email provider is required to be configured
    // (see lib/email/client.ts's log fallback), so a new account could never
    // receive its verification link and would be locked out forever. Skip the
    // inbox-ownership proof in dev/test only; production always requires it.
    const autoVerifiedForDev = !isProduction;
    ...
    const user = await prisma.user.create({
      data: {
        ...
        passwordHash,
        name: validated.name,
        emailVerified: verifiedByInvitation || autoVerifiedForDev ? new Date() : null,
      },
      select: {
        id: true,
        email: true,
