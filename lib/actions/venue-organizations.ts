...
export async function updateVenueProfile(
  ...
  try {
    ...
    if (venue.slug) {
      revalidatePath(`/circuits/${venue.slug}`);
    }
    if (updated.slug && updated.slug !== venue.slug) {
      revalidatePath(`/circuits/${updated.slug}`);
    }
    revalidatePath("/circuits");
...
export async function publishVenueProfile(
  ...
  try {
    ...
    if (slugConflict) {
      return { success: false, error: "That public circuit slug is already in use" };
    }
...
export async function getPublicCircuitSummaries() {
  try {
    ...
    if (isMissingVenueManagementSchemaError(error)) {
      console.warn("Public circuit listing unavailable until venue management migration is applied", {
        code: error.code,
      });
      return [];
    }
...
export async function getPublicVenueProfile(slug: string) {
  try {
    const now = new Date();
    ...
    if (isMissingVenueManagementSchemaError(error)) {
      console.warn("Public circuit profile unavailable until venue management migration is applied", {
        code: error.code,
        slug,
      });
      return null;
