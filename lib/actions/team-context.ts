...
type TeamRosterData = {
  teamId: string;
  teamName: string;
  /** Drives sport-specific roster fields (e.g. motorsport licence/car number). */
  sport: Sport;
  isAdmin: boolean;
  players: Player[];
  teamMembers: Array<{
  ...
  const [players, teamMembers, rawInvitations] = await Promise.all([
    prisma.player.findMany({
      ...
      select: {
        ...
        jerseyNumber: true,
        position: true,
        // The FMSCI licence number is a governing-body identifier, so it is
        // admin-only (FR-008) and must not appear in non-admin queries. Car number and class are roster
        // identity everyone on the team sees, like jerseyNumber and position.
        fmsciLicenseNumber: isAdmin,
        carNumber: true,
        racingClass: true,
 ...
 */
export async function getRosterData(): Promise<TeamRosterData | null> {
  const userId = await requireUserId();
 ...
 */
export async function getTeamRosterDataById(teamId: string): Promise<TeamRosterData | null> {
  const team = await getAccessibleTeamData(teamId);
  if (!team) return null;
