...
import {
  ...
  type SubmitSurfaceTimeRequestInput,
} from "@/lib/utils/validation";
import {
  sendSurfaceTimeRequestDecisionEmail,
  sendSurfaceTimeRequestSubmittedEmail,
...
export type DecideSurfaceTimeRequestTransactionHooks = {
  beforeCreateReservation?: () => Promise<void>;
};

export async function decideSurfaceTimeRequestInTransaction(
  tx: Prisma.TransactionClient,
  validated: z.infer<typeof decisionSchema>,
  userId: string,
  hooks: DecideSurfaceTimeRequestTransactionHooks = {},
) {
  ...
  const request = await tx.surfaceTimeRequest.findFirst({
    where: {
  ...
  if (!request) {
    return { success: false as const, error: "Track time request not found" };
  }
  ...
  if (
    ...
    return {
      ...
      error:
        "This track time request already has a different final decision.",
    };
  }
  ...
  if (approves && !idempotentDecision) {
    ...
    if (legacyAcceptedConflicts.length > 0 && !validated.overrideConflicts) {
      return { success: false as const, error: "That track time has already been accepted for another request" };
    }
  ...
  const updated = idempotentDecision
    ...
    : await tx.surfaceTimeRequest.update({
        where: { id: request.id },
...
export async function submitSurfaceTimeRequest(
  input: SubmitSurfaceTimeRequestInput
): Promise<ActionResult<{ requestId: string; status: string }>> {
  try {
    const validated = submitSurfaceTimeRequestSchema.parse(input);
    ...
    if (validated.requesterLeagueId) {
      ...
      if (!role) {
        return { success: false, error: "You are not authorized to request track time for that league" };
      }
    }
    ...
    if (!block) {
      return { success: false, error: "Available track time block not found" };
    }

    if (block.registrationMode !== "REQUEST_REQUIRED") {
      return { success: false, error: "That track-time block is not accepting track time requests" };
    }
    ...
    if (!requestedWithinOffering) {
      return { success: false, error: "Requested time must be within the published available track time block" };
    }

    const request = await prisma.surfaceTimeRequest.create({
      data: {
    ...
    if (organizationId && managerEmails.length > 0) {
      await sendSurfaceTimeRequestSubmittedEmail({
        managerEmails,
    ...
    return { success: false, error: "Failed to submit track time request." };
  }
}

export async function decideSurfaceTimeRequest(
  ...
  try {
    ...
    const decision = await runVenueReservationTransaction((tx) =>
      decideSurfaceTimeRequestInTransaction(tx, validated, userId));
    ...
    if (
      ...
      await sendSurfaceTimeRequestDecisionEmail({
    ...
    if (error instanceof Prisma.PrismaClientKnownRequestError && error.code === "P2034") {
      return {
        success: false,
        error: "That track time was updated by another manager. Please review the queue and try again.",
      };
    }
    return { success: false, error: "Failed to update track time request." };
  }
}

export async function cancelSurfaceTimeRequest(
  input: z.input<typeof requestCommandSchema>
): Promise<ActionResult<{ requestId: string; status: string }>> {
  return setRequestStatus(input, "CANCELED");
}

export async function expireSurfaceTimeRequest(
  input: z.input<typeof requestCommandSchema>
): Promise<ActionResult<{ requestId: string; status: string }>> {
  return setRequestStatus(input, "EXPIRED");
}

export async function annotateSurfaceTimeRequest(
  ...
  try {
    ...
    const updated = await runVenueReservationTransaction(async (tx) => {
      ...
      const row = await tx.surfaceTimeRequest.update({
        where: { id: request.id },
        data: {
    ...
    if (!updated?.decisionMessage) {
      return { success: false, error: "Track time request not found" };
    }
    ...
    return { success: false, error: "Failed to annotate track time request." };
  }
}
...
> {
  try {
    ...
    const requests = await (prisma.surfaceTimeRequest.findMany as any)({
      where: {
        venueId,
  ...
  try {
    ...
    const updated = await runVenueReservationTransaction(async (tx) => {
      ...
      if (
        ...
        throw new VenueReservationLifecycleError(
          "This track time request already has a different final status.",
        );
      }
      if (
        ...
        await transitionVenueReservation(tx, {
          ...
          nextStatus: "RELEASED",
          actorId,
          reason: `Source track time request ${status.toLowerCase()}`,
    ...
    if (!updated) return { success: false, error: "Track time request not found" };
...
async function findManagedRequest(organizationId: string, venueId: string, requestId: string) {
  return prisma.surfaceTimeRequest.findFirst({
    where: {
      id: requestId,
      venueId,
      venue: { organizationId },
...
function revalidateRequestPaths(organizationId: string | null, venueId: string, slug?: string | null) {
  ...
  if (slug) {
    revalidatePath(`/circuits/${slug}/schedule`);
  }
