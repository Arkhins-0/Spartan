...
import {
  ...
  updateVenueSurfaceSchema,
  venueOperatingHourSchema,
  venueScheduleBlockSchema,
  type CreateVenueSurfaceInput,
  type UpdateVenueSurfaceInput,
...
const scheduleBlockIdSchema = venueScheduleBlockSchema.extend({
  scheduleBlockId: createVenueSurfaceSchema.shape.venueId,
});

const scheduleBlockCommandSchema = createVenueSurfaceSchema.pick({
  organizationId: true,
  venueId: true,
}).extend({
  scheduleBlockId: createVenueSurfaceSchema.shape.venueId,
...
const surfaceCommandSchema = createVenueSurfaceSchema.pick({
  organizationId: true,
  venueId: true,
}).extend({
  surfaceId: updateVenueSurfaceSchema.shape.surfaceId,
});

const operatingHourCommandSchema = createVenueSurfaceSchema.pick({
  organizationId: true,
  venueId: true,
}).extend({
  operatingHourId: createVenueSurfaceSchema.shape.venueId,
});
...
export async function getVenueScheduleAdminData(
  ...
  ActionResult<{
    ...
    availableSurface: VenueReservationOfferingWithAvailability[];
  }>
> {
  try {
    ...
    const availableSurface = await populateVenueOfferingAvailability(prisma, {
      venueId,
...
export async function createVenueSurface(
  input: CreateVenueSurfaceInput
): Promise<ActionResult<{ surfaceId: string; venueId: string }>> {
  try {
    const validated = createVenueSurfaceSchema.parse(input);
    ...
    const surface = await prisma.venueSurface.create({
      data: {
    ...
    return { success: false, error: "Failed to create surface." };
  }
}

export async function updateVenueSurface(
  input: UpdateVenueSurfaceInput
): Promise<ActionResult<{ surfaceId: string; venueId: string }>> {
  try {
    const validated = updateVenueSurfaceSchema.parse(input);
    const userId = await requireVenueScheduleManager(validated.organizationId, validated.venueId);
    await ensureVenueContext(validated.organizationId, validated.venueId);

    const outcome = await runVenueReservationTransaction(async (tx) => {
      const existing = await tx.venueSurface.findFirst({
      ...
      const surface = await tx.venueSurface.update({
        where: { id: validated.surfaceId, venueId: validated.venueId },
        data: {
          name: validated.name,
          surfaceType: validated.surfaceType,
    ...
    return { success: false, error: "Failed to update surface." };
  }
}

export async function archiveVenueSurface(input: {
  organizationId: string;
  venueId: string;
  surfaceId: string;
}): Promise<ActionResult<{ surfaceId: string; venueId: string }>> {
  try {
    ...
    return { success: false, error: "Failed to archive surface." };
  }
}
...
/**
 * Future bookings that reference a surface, across the three surface-capable
 * sources (SeasonGame, EventGame, VenueScheduleBlock) —
...
async function findFutureSurfaceBookings(
  ...
  const [reservations, seasonGames, eventGames, blocks, requests] = await Promise.all([
    ...
    client.surfaceTimeRequest.findMany({
      where: {
        venueId: scope.venueId,
  ...
  for (const request of requests) {
    ...
    bookings.push({
      id: request.id,
      source: "surfaceTimeRequest" as VenueBookingView["source"],
