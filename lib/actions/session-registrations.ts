 ...
 * the circuit's connected account) and return a checkout URL.
 */
export async function registerForSession(
  ...
  try {
    ...
    if (!offering.organization?.stripeAccountId || !offering.organization.stripeChargesEnabled) {
      return { success: false, error: "This circuit hasn't finished setting up online payments yet." };
    }
    ...
    const successPath = slug
      ? `/circuits/${slug}/schedule?registration=success`
      : `/my-registrations?registration=success`;
    const cancelPath = slug
      ? `/circuits/${slug}/schedule?registration=canceled`
 ...
 * immediately; paid & confirmed registrations must be refunded by the circuit.
 */
export async function cancelMyRegistration(
  ...
  try {
    ...
    if (registration.amountTotal > 0 && registration.status === "CONFIRMED") {
      return { success: false, error: "This is a paid registration — contact the circuit to request a refund." };
    }
    ...
    if (registration.venue.slug) {
      revalidatePath(`/circuits/${registration.venue.slug}/schedule`);
    }
...
/** List the current user's registrations across all circuits. */
...
function revalidateRegistrationPaths(offering: RegistrableOffering) {
  if (offering.venue.slug) {
    revalidatePath(`/circuits/${offering.venue.slug}/schedule`);
    revalidatePath(`/circuits/${offering.venue.slug}`);
  }
  revalidatePath("/my-registrations");
