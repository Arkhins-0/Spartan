 ...
 * surface or one of its named segments (e.g. a half or sector zone).
...
export async function upsertEventGame(
  ...
  try {
    ...
    if (surfaceId) {
      const surface = await prisma.venueSurface.findFirst({
    ...
    if (event.venueId && (prisma as typeof prisma & { venueReservation?: unknown }).venueReservation) {
      try {
        const result = await runVenueReservationTransaction(async (tx) => {
          ...
          // publishes the parent, so a draft can never occupy confirmed track time.
...
export async function setGameRotation(
  ...
  try {
    ...
    if (invalidSide) {
      return { success: false, error: "Rotation entries must drive for one of this game's two teams." };
    }
 ...
 * circuit's public schedule page alongside the event listing (FR-032).
 */
 ...
 * at or above the configured age threshold. No scores or statistics
 * below the configured age level; the gate is enforced here and again in
 * every read (FR-036).
 */
