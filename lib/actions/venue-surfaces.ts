 ...
 * paths plus the public circuit pages.
 */
 ...
 * omitted label resets to the surface-type default ("Full circuit", …).
 */
export async function setWholeSurfaceLabel(
  ...
  try {
    ...
    await prisma.venueSurface.update({
...
async function requireSurfaceManager(surfaceId: string): Promise<SurfaceContext> {
  const surface = await prisma.venueSurface.findUnique({
    where: { id: surfaceId },
    select: {
      id: true,
      name: true,
...
/**
 * Future bookings that reference the given segments, across the three
 * segment-capable sources (SeasonGame, EventGame, VenueScheduleBlock).
 * Calendar Events are always venue-wide — they can never
...
function revalidateSegmentationPaths(context: {
  ...
  if (context.slug) {
    revalidatePath(`/circuits/${context.slug}`);
    revalidatePath(`/circuits/${context.slug}/schedule`);
  }
}
