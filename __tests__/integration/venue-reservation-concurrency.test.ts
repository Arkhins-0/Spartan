...
describeWithTestDatabase("venue reservation concurrency (T018)", () => {
  ...
  let decideSurfaceTimeRequestInTransaction: any;
  ...
  beforeAll(async () => {
    ...
    decideSurfaceTimeRequestInTransaction =
      actionModule.decideSurfaceTimeRequestInTransaction;
  ...
  async function createFixture(): Promise<Fixture> {
    ...
    const [requestA, requestB] = await Promise.all([
      prisma.surfaceTimeRequest.create({
        data: {
  ...
  it("lets only one overlapping non-coexisting approval commit", async () => {
    ...
    const requestStatuses = await prisma.surfaceTimeRequest.findMany({
