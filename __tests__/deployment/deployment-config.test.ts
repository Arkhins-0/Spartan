...
describe('deployment configuration', () => {
  ...
  it('publishes documentation to GitHub Pages with the custom domain artifact', async () => {
    ...
    expect(docsBuilder).toContain('dev.arkhins.com/spartan');
  ...
  it('runs scheduled uptime monitoring for the main and docs sites', async () => {
    ...
    expect(DEFAULT_UPTIME_TARGETS).toEqual([
      ...
      { name: 'docs', url: 'https://dev.arkhins.com/spartan' },
    ]);
  });
});
...
describe('uptime checker', () => {
  ...
  it('parses named target overrides', () => {
    expect(parseUptimeTargets('main=https://spartan.arkhins.com,docs=https://dev.arkhins.com/spartan/status')).toEqual([
      { name: 'main', url: 'https://spartan.arkhins.com' },
      { name: 'docs', url: 'https://dev.arkhins.com/spartan/status' },
    ]);
  });
  ...
  it('marks network errors as failing', async () => {
    ...
    const [result] = await checkUptimeTargets([{ name: 'docs', url: 'https://dev.arkhins.com/spartan' }], {
      fetcher,
      timeoutMs: 1_000,
    });
