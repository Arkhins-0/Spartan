...
describe('SEO Configuration', () => {
    describe('SITE_CONFIG', () => {
        it('should have all required site configuration properties', () => {
            expect(SITE_CONFIG.name).toBe('Spartan');
            expect(SITE_CONFIG.title).toBeTruthy();
            expect(SITE_CONFIG.description).toBeTruthy();
            expect(SITE_CONFIG.url).toBe('https://spartan.arkhins.com');
        ...
        it('should include relevant keywords for motorsport championship management', () => {
            const keywords = SITE_CONFIG.keywords;
            expect(keywords).toContain('motorsport championship management');
            expect(keywords).toContain('race team management');
            expect(keywords).toContain('entry list management');
        });
    });
    ...
    describe('generatePageMetadata', () => {
        it('should generate basic metadata with title and description', () => {
            ...
            expect(metadata.title).toBe('Test Page - Spartan');
            expect(metadata.description).toBe('Test description');
        });

        it('should not duplicate Spartan in title if already present', () => {
            const metadata = generatePageMetadata({
                title: 'Spartan - Test',
                description: 'Test description',
            });

            expect(metadata.title).toBe('Spartan - Test');
        });
        ...
        it('should generate Open Graph metadata', () => {
            ...
            expect((metadata.openGraph as any)?.title).toBe('Features - Spartan');
            expect((metadata.openGraph as any)?.description).toBe('Feature description');
            expect((metadata.openGraph as any)?.url).toBe('https://spartan.arkhins.com/features');
            expect((metadata.openGraph as any)?.siteName).toBe('Spartan');
        });
        ...
        it('should generate Twitter Card metadata', () => {
            ...
            expect((metadata.twitter as any)?.title).toBe('Pricing - Spartan');
            expect((metadata.twitter as any)?.description).toBe('Pricing description');
            // No X/Twitter account is claimed, so the card omits site/creator.
            expect((metadata.twitter as any)?.site).toBeUndefined();
        });
        ...
        it('should merge custom keywords with site keywords', () => {
            ...
            expect(keywordsString).toContain('motorsport championship management');
        });
        ...
        it('should set canonical URL correctly', () => {
            ...
            expect(metadata.alternates?.canonical).toBe('https://spartan.arkhins.com/about');
        });
        ...
        it('should use custom OG image when provided', () => {
            ...
            expect(ogImages).toEqual([{ url: 'https://spartan.arkhins.com/custom-image.png', width: 1200, height: 630, alt: 'Custom - Spartan' }]);
            expect(twitterImages).toEqual(['https://spartan.arkhins.com/custom-image.png']);
        });
    });
    ...
    describe('Structured Data Schemas', () => {
        describe('getOrganizationSchema', () => {
            it('should generate valid Organization schema', () => {
                ...
                expect(schema.name).toBe('Spartan');
                expect(schema.url).toBe('https://spartan.arkhins.com');
        ...
        describe('getSoftwareApplicationSchema', () => {
            ...
            it('should include feature list as string per Schema.org spec', () => {
                ...
                expect(schema.featureList).toContain('Entry List Management');
                expect(schema.featureList).toContain('Race Weekend Scheduling');
                expect(schema.featureList).toContain('Marshal Signup');
            });
        ...
        describe('getBreadcrumbSchema', () => {
            ...
            it('should include full URLs for breadcrumb items', () => {
                ...
                expect(schema.itemListElement[0].item).toBe('https://spartan.arkhins.com/features');
            });
        });
        ...
        describe('getFAQSchema', () => {
            it('should generate valid FAQPage schema', () => {
                const faqs = [
                    { question: 'What is Spartan?', answer: 'A team management platform' },
