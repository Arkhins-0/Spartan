...
describe("sanitizeHtml", () => {
  it("keeps the legacy function name as an alias for HTML escaping", () => {
    expect(sanitizeHtml("<strong>Spartan</strong>")).toBe(
      escapeHtml("<strong>Spartan</strong>"),
    );
  });
});
...
describe("sanitizeUrl", () => {
  it("allows safe absolute and relative URLs", () => {
    expect(sanitizeUrl("https://spartan.test/rinks?q=ice time")).toBe(
      "https://spartan.test/rinks?q=ice%20time",
    );
  ...
  it("can require absolute URLs with custom protocol allowlists", () => {
    ...
    expect(sanitizeUrl("https://spartan.test", { allowedProtocols: ["mailto"] })).toBeNull();
  });
