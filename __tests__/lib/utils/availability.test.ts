...
describe("availability engine", () => {
  ...
  describe("findBookingConflicts — SC-004 segment matrix", () => {
    it("does not conflict when the candidate is on a segment that coexists with the booked one (all segment-capable sources)", async () => {
      ...
      // other three sources are seeded on A while the candidate takes B.
    ...
    it("treats rows without a surface as venue-wide, conflicting with any segment candidate", async () => {
      ...
      expect(conflicts.map((c) => c.source)).toEqual(["scheduleBlock"]);
    });
  ...
  describe("findBookingConflicts — exclusions", () => {
    ...
    describe("feature 007 dual-read compatibility", () => {
      ...
      it("excludes the canonical reservation linked to an edited legacy source", async () => {
        reservationRows = [reservationRow({ seasonGameId: "game-1" })];
        seasonGameRows = [gameRow({ venueReservationId: "venue-reservation-1" })];
  ...
  describe("performance (SC-008)", () => {
    it("keeps p95 latency under 500ms across 50 runs against 800 bookings", async () => {
      // 800 bookings at one venue over one month: 200 per source, spread
