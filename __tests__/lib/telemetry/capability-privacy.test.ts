...
describe('isPublicCapabilityUrl', () => {
  it('matches absolute and relative capability URLs', () => {
    expect(isPublicCapabilityUrl(`https://spartan.arkhins.com/gear-wishlist/${TOKEN}`)).toBe(true);
    expect(isPublicCapabilityUrl(`/gear-wishlist/${TOKEN}`)).toBe(true);
  });

  it('resolves against a base URL', () => {
    expect(isPublicCapabilityUrl(`gear-wishlist/${TOKEN}`, 'https://spartan.arkhins.com/')).toBe(true);
  });

  it('does not match other routes', () => {
    expect(isPublicCapabilityUrl('https://spartan.arkhins.com/dashboard')).toBe(false);
...
describe('scrubCapabilityTokens', () => {
  it('redacts the token from an absolute URL', () => {
    expect(scrubCapabilityTokens(`https://spartan.arkhins.com/gear-wishlist/${TOKEN}`)).toBe(
      `https://spartan.arkhins.com/gear-wishlist/${CAPABILITY_TOKEN_REDACTION}`
    );
  });
  ...
  it('redacts a percent-encoded token carried inside a query parameter', () => {
    expect(
      scrubCapabilityTokens(`https://spartan.arkhins.com/login?callbackUrl=%2Fgear-wishlist%2F${TOKEN}`)
    ).toBe(
      `https://spartan.arkhins.com/login?callbackUrl=%2Fgear-wishlist%2F${CAPABILITY_TOKEN_REDACTION}`
    );
  });
...
describe('scrubTelemetryPayload', () => {
  it('redacts tokens throughout a Sentry-shaped event', () => {
    const event = {
      ...
      request: {
        url: `https://spartan.arkhins.com/gear-wishlist/${TOKEN}?ref=email`,
        headers: { Referer: `https://spartan.arkhins.com/gear-wishlist/${TOKEN}` },
      },
      breadcrumbs: [
