...
const { mockRequireUserId, mockRequireTeamMember, mockGetUserLeagueRole, mockPrisma } = vi.hoisted(() => ({
  ...
  mockPrisma: {
    ...
    surfaceTimeRequest: { create: vi.fn(), findFirst: vi.fn() },
...
vi.mock("@/lib/email/templates", () => ({
  sendSurfaceTimeRequestSubmittedEmail: vi.fn(),
}));

import { submitSurfaceTimeRequest } from "@/lib/actions/venue-requests";
...
beforeEach(() => {
  ...
  mockPrisma.surfaceTimeRequest.create.mockResolvedValue({ id: "clreqxxxxxxxxxxxxxxxxxxxxxxx", status: "SUBMITTED" });
  mockPrisma.venueStaff.findMany.mockResolvedValue([]);
});

describe("track time request authorization and booking safety", () => {
  ...
  it("allows competing requests because canonical conflicts are decided atomically at approval", async () => {
    mockPrisma.surfaceTimeRequest.findFirst.mockResolvedValue({ id: "existing-request" });
    ...
    expect(result.success).toBe(true);
    expect(mockPrisma.surfaceTimeRequest.create).toHaveBeenCalled();
  });
});
