...
const { mockRequireUserId, mockGetCurrentUserId, mockIsEventManager, mockDeleteObject, mockPrisma } = vi.hoisted(() => ({
...
// Storage seam driven through the Vercel Blob provider's real reference
// validation; configuration and side effects are stubbed.
...
describe("finalizeEventMediaUpload", () => {
  ...
  it("rejects blobs stored outside this event's prefix", async () => {
    const result = await finalizeEventMediaUpload({
      eventId: EVENT_ID,
      key: "https://abc123.public.blob.vercel-storage.com/signup-events/other-event/photo.jpg",
      contentType: "image/jpeg",
      sizeBytes: 1024,
    });
  ...
  it("rejects external hosts even when the path contains the event prefix", async () => {
    const result = await finalizeEventMediaUpload({
      eventId: EVENT_ID,
      key: `https://attacker.example/x/signup-events/${EVENT_ID}/fake.png`,
      contentType: "image/jpeg",
      sizeBytes: 1024,
    });
  ...
  it("anchors the prefix check to the start of the pathname", async () => {
    const result = await finalizeEventMediaUpload({
      eventId: EVENT_ID,
      key: `https://abc123.public.blob.vercel-storage.com/elsewhere/signup-events/${EVENT_ID}/fake.png`,
      contentType: "image/jpeg",
      sizeBytes: 1024,
    });
...
describe("removeEventMediaItem", () => {
  ...
  it("lets uploaders delete their own item (object removed best-effort)", async () => {
    mockRequireUserId.mockResolvedValue("parent-1");
    mockIsEventManager.mockResolvedValue(false);
    ...
    expect(mockDeleteObject).toHaveBeenCalledWith({ provider: "vercel-blob", key: BLOB_URL });
