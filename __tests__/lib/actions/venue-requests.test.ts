...
const {
  ...
  // T023/T024 (unbuilt): `decideSurfaceTimeRequest`/cancel/expire are expected to
  // start calling the already-implemented reservation service atomically
  // alongside the `SurfaceTimeRequest` row update. Mocked here as black boxes
  ...
  mockPrisma: {
    ...
    surfaceTimeRequest: { create: vi.fn(), findFirst: vi.fn(), update: vi.fn(), findMany: vi.fn() },
...
vi.mock("@/lib/email/templates", () => ({
  sendSurfaceTimeRequestSubmittedEmail: (...args: unknown[]) => mockSubmitEmail(...args),
  sendSurfaceTimeRequestDecisionEmail: (...args: unknown[]) => mockDecisionEmail(...args),
}));
...
import {
  cancelSurfaceTimeRequest,
  decideSurfaceTimeRequest,
  expireSurfaceTimeRequest,
  getVenueRequestQueue,
  submitSurfaceTimeRequest,
...
beforeEach(() => {
  ...
  mockPrisma.surfaceTimeRequest.findFirst.mockResolvedValue(null);
...
describe("track time request lifecycle", () => {
  it("submits a request for a published schedule block", async () => {
    mockPrisma.surfaceTimeRequest.create.mockResolvedValue({ id: REQUEST_ID, status: "SUBMITTED" });

    const result = await submitSurfaceTimeRequest({
    ...
    expect(mockPrisma.surfaceTimeRequest.create).toHaveBeenCalledWith(
  ...
  it("submits a request for a concrete recurring offering occurrence", async () => {
    ...
    mockPrisma.surfaceTimeRequest.create.mockResolvedValue({
      id: REQUEST_ID,
      status: "SUBMITTED",
    });
  ...
  it("lets venue request managers accept, cancel, and expire requests", async () => {
    ...
    mockPrisma.surfaceTimeRequest.findFirst
      .mockResolvedValueOnce(managedRequest)
      .mockResolvedValue(managedRequest);
    mockPrisma.surfaceTimeRequest.update
    ...
    expect((await decideSurfaceTimeRequest({ organizationId: ORGANIZATION_ID, venueId: VENUE_ID, requestId: REQUEST_ID, status: "ACCEPTED" })).success).toBe(true);
    expect(mockPrisma.$transaction).toHaveBeenCalledWith(expect.any(Function), expect.objectContaining({
      isolationLevel: "Serializable",
    }));
    expect((await cancelSurfaceTimeRequest({ organizationId: ORGANIZATION_ID, venueId: VENUE_ID, requestId: REQUEST_ID })).success).toBe(true);
    expect((await expireSurfaceTimeRequest({ organizationId: ORGANIZATION_ID, venueId: VENUE_ID, requestId: REQUEST_ID })).success).toBe(true);
  ...
  it("prevents accepting a request that overlaps an accepted request", async () => {
    mockPrisma.surfaceTimeRequest.findFirst.mockResolvedValueOnce(
    ...
    const result = await decideSurfaceTimeRequest({
      organizationId: ORGANIZATION_ID,
      venueId: VENUE_ID,
    ...
    expect(mockPrisma.surfaceTimeRequest.update).not.toHaveBeenCalled();
  });
  ...
  it("does not bluntly reject a new request because an accepted request exists", async () => {
    ...
    expect(result.success).toBe(true);
    expect(mockPrisma.surfaceTimeRequest.create).toHaveBeenCalledOnce();
  });
  ...
  it("returns private request details for the manager queue", async () => {
    ...
    expect(result.success).toBe(true);
    expect(mockPrisma.surfaceTimeRequest.findMany).toHaveBeenCalledWith(
      expect.objectContaining({
 ...
 * `decideSurfaceTimeRequest`/`cancelSurfaceTimeRequest`/`expireSurfaceTimeRequest`, which
 * today only ever mutate the `SurfaceTimeRequest` row and never touch a
...
describe("decideSurfaceTimeRequest full/partial/decline approval (T017 -> T023 intended contract)", () => {
  beforeEach(() => {
  ...
  it("fully approves a request, creates exactly one confirmed reservation, and returns queued notification identifiers", async () => {
    mockPrisma.surfaceTimeRequest.findFirst.mockResolvedValueOnce(managedRequestFixture());
    mockPrisma.surfaceTimeRequest.update.mockResolvedValueOnce({
      id: REQUEST_ID,
      status: "ACCEPTED",
  ...
  it("treats a narrower approved interval as PARTIALLY_ACCEPTED even when the caller submits ACCEPTED", async () => {
    ...
    expect(result.success).toBe(true);
    expect(mockPrisma.surfaceTimeRequest.update).toHaveBeenCalledWith(
      expect.objectContaining({
...
describe("cancel/expire track time requests propagate to the linked reservation (T017 -> T024 intended contract)", () => {
  it("cancelSurfaceTimeRequest releases the linked reservation atomically instead of leaving it orphaned", async () => {
    mockPrisma.surfaceTimeRequest.findFirst.mockResolvedValueOnce(
      managedRequestFixture({ venueReservation: { id: RESERVATION_ID, status: "CONFIRMED" } }),
    );
    mockPrisma.surfaceTimeRequest.update.mockResolvedValueOnce({ id: REQUEST_ID, status: "CANCELED" });
    mockTransitionVenueReservation.mockResolvedValueOnce({ id: RESERVATION_ID, status: "RELEASED" });

    const result = await cancelSurfaceTimeRequest({
      organizationId: ORGANIZATION_ID,
      venueId: VENUE_ID,
      requestId: REQUEST_ID,
    });
  ...
  it("expireSurfaceTimeRequest releases the linked reservation the same way cancellation does", async () => {
    mockPrisma.surfaceTimeRequest.findFirst.mockResolvedValueOnce(
      managedRequestFixture({ venueReservation: { id: RESERVATION_ID, status: "CONFIRMED" } }),
    );
    mockPrisma.surfaceTimeRequest.update.mockResolvedValueOnce({ id: REQUEST_ID, status: "EXPIRED" });
    mockTransitionVenueReservation.mockResolvedValueOnce({ id: RESERVATION_ID, status: "RELEASED" });

    const result = await expireSurfaceTimeRequest({
      organizationId: ORGANIZATION_ID,
      venueId: VENUE_ID,
      requestId: REQUEST_ID,
    });
  ...
  it("cancelSurfaceTimeRequest releases the replacement reservation now linked after a reschedule", async () => {
  ...
  it("expireSurfaceTimeRequest releases the replacement reservation now linked after a reschedule", async () => {
    const replacementReservationId = "clresreplacementxxxxxxxxxx";
    mockPrisma.surfaceTimeRequest.findFirst.mockResolvedValueOnce(
      managedRequestFixture({
        status: "PARTIALLY_ACCEPTED",
  ...
  it("cancelling a request with no linked reservation still succeeds without calling the reservation service", async () => {
    mockPrisma.surfaceTimeRequest.findFirst.mockResolvedValueOnce(managedRequestFixture({ venueReservation: null }));
    mockPrisma.surfaceTimeRequest.update.mockResolvedValueOnce({ id: REQUEST_ID, status: "CANCELED" });
...
describe("exact auth / cross-tenant protections on track time request decisions", () => {
  it("scopes decideSurfaceTimeRequest's lookup to the exact organization and venue (existing protection)", async () => {
    mockPrisma.surfaceTimeRequest.findFirst.mockResolvedValueOnce(null);
    ...
    expect(result.success).toBe(false);
    expect(mockPrisma.surfaceTimeRequest.findFirst).toHaveBeenCalledWith(
      expect.objectContaining({
        where: expect.objectContaining({
          id: REQUEST_ID,
          venueId: VENUE_ID,
  ...
  it("checks venue-request-manager authority for the exact organization/venue pair before deciding (existing protection)", async () => {
    ...
    await decideSurfaceTimeRequest({
      organizationId: ORGANIZATION_ID,
      venueId: VENUE_ID,
      requestId: REQUEST_ID,
      status: "ACCEPTED",
  ...
  it("rejects a decision attempt when the caller's venue-request authority is scoped to a different venue (cross-tenant)", async () => {
    ...
    expect(result.success).toBe(false);
    expect(mockPrisma.surfaceTimeRequest.findFirst).not.toHaveBeenCalled();
  });
});
