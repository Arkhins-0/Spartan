...
function tx() {
  return {
    ...
    surfaceTimeRequest: { findMany: vi.fn().mockResolvedValue([]) },
...
describe("venue reservation availability", () => {
  ...
  it("applies canonical split-segment coexistence to legacy accepted requests", async () => {
    const client = tx();
    vi.mocked(client.surfaceTimeRequest.findMany).mockResolvedValue([{
      id: "legacy-request",
    ...
    expect(client.surfaceTimeRequest.findMany).toHaveBeenCalledWith(
      expect.objectContaining({
        where: expect.objectContaining({
