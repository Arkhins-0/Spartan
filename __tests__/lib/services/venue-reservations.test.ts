...
describe("createVenueReservation validation", () => {
  ...
  it.each([
    ...
    ["surface", (tx: Prisma.TransactionClient) =>
      vi.mocked(tx.venueSurface.findFirst).mockResolvedValue(null)],
  ...
  it.each([
    ...
    vi.mocked(tx.surfaceTimeRequest.findUnique).mockResolvedValue({
      status: "ACCEPTED",
      venueId: "venue-1",
  ...
  it("creates only after an accepted matching request is fully validated", async () => {
    ...
    expect(tx.surfaceTimeRequest.findUnique).toHaveBeenCalledBefore(
...
describe("assignVenueReservation validation", () => {
  ...
  it("allows league authority to assign league-owned inventory to a team in that league", async () => {
    const tx = makeTx();
    const targetId = setAssignmentTarget(tx, "EVENT");
  ...
  it("rechecks conflicts for an already-linked target", async () => {
    ...
    const targetId = setAssignmentTarget(tx, "EVENT", "league-1", venueReservationId);
  ...
  it("preserves explicit reservation exclusions during assignment rechecks", async () => {
    const tx = makeTx();
    const targetId = setAssignmentTarget(tx, "EVENT", "league-1", null);
