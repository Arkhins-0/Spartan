...
describe('PricingPage', () => {
  ...
  it('states commitments and the repositioned pricing FAQ without future-bill hedging', () => {
    ...
    expect(screen.getByRole('heading', { level: 3, name: /how does spartan make money/i })).toBeInTheDocument();
  });

  it('sets indexable SEO metadata with canonical pricing URL', () => {
    expect(metadata.title).toBe('Pricing - Spartan');
    expect(metadata.description).toMatch(/free forever for teams/i);
    expect(metadata.alternates?.canonical).toBe('https://spartan.arkhins.com/pricing');
