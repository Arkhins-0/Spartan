...
import { DraggableTeamCard, DraggableTeamRow } from '@/components/features/team/DraggableTeamCard';
...
describe('DraggableTeamRow', () => {
  it('renders a table row with a labelled drag handle in the first cell', () => {
    render(
      <DndContext>
        <table>
          <tbody>
            <DraggableTeamRow id="team-1" handleLabel="Drag Team Alpha">
              <td data-testid="row-content">Team Alpha</td>
            </DraggableTeamRow>
          </tbody>
        </table>
      </DndContext>
    ...
    const handle = screen.getByLabelText('Drag Team Alpha');
    expect(handle).toBeInTheDocument();
    expect(handle.closest('td')).toBe(screen.getByRole('row').firstElementChild);
    expect(screen.getByTestId('row-content')).toBeInTheDocument();
  });

  it('forwards row props such as click handlers', async () => {
    const onClick = vi.fn();
    render(
      <DndContext>
        <table>
          <tbody>
            <DraggableTeamRow id="team-1" handleLabel="Drag Team Alpha" onClick={onClick}>
              <td>Team Alpha</td>
            </DraggableTeamRow>
          </tbody>
        </table>
