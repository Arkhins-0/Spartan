...
describe('HeroSection', () => {
  ...
  describe('Basic Rendering', () => {
    it('renders main headline with Spartan branding', () => {
      renderWithTheme(<HeroSection />);

      expect(screen.getByRole('heading', { level: 1 })).toBeInTheDocument();
      // BrandLogo displays image instead of text
      expect(screen.getByAltText('Spartan - Simplify Your Season')).toBeInTheDocument();
      expect(screen.getByText(/Simplify Your Season/)).toBeInTheDocument();
      expect(screen.getByText(/Race More/)).toBeInTheDocument();
    });

    it('renders value proposition subtitle', () => {
      renderWithTheme(<HeroSection />);

      expect(screen.getByText(/The free, open-source championship management platform/)).toBeInTheDocument();
      expect(screen.getByText(/one organised paddock for entry lists/)).toBeInTheDocument();
    });
    ...
    it('renders brand logo', () => {
      ...
      const brandLogo = screen.getByAltText('Spartan - Simplify Your Season');
    ...
    it('renders additional trust signal text', () => {
      ...
      expect(screen.getByText(/Built by racers, for racers/)).toBeInTheDocument();
    });
  });
  ...
  describe('Content Structure and Accessibility', () => {
    it('has proper heading hierarchy', () => {
      ...
      expect(mainHeading).toHaveTextContent(/Simplify Your Season.*Race More/);
    });
    ...
    it('provides meaningful text content for screen readers', () => {
      ...
      expect(screen.getByText(/open-source championship management platform/)).toBeInTheDocument();
      expect(screen.getByText(/built for race organisers/)).toBeInTheDocument();
      expect(screen.getByText(/entry lists, race weekends, session timetables, and communication/)).toBeInTheDocument();
    });
  ...
  describe('Visual Design Elements', () => {
    it('paints the section on the page token with no gradient', () => {
      renderWithTheme(<HeroSection />);

      const heroSection = screen.getByRole('region', { name: /simplify your season/i });
      const styles = getComputedStyle(heroSection);
      expect(styles.backgroundColor).toContain('--mui-palette-background-default');
      expect(styles.backgroundImage || '').not.toContain('gradient');
    });

    it('renders Spartan brand logo', () => {
      renderWithTheme(<HeroSection />);
