  ...
  it('renders testimonial section copy and cards', () => {
    ...
    expect(
      screen.getByRole('heading', {
        level: 2,
        name: /trusted by the people who keep championships moving/i,
      })
    ).toBeInTheDocument();
    expect(screen.getByText(/representative feedback, transparent community signals/i)).toBeInTheDocument();
    expect(screen.getAllByRole('article')).toHaveLength(3);
    expect(screen.getByText('Club championship organiser')).toBeInTheDocument();
    expect(screen.getByText('Team manager')).toBeInTheDocument();
    expect(screen.getByText('Circuit operator')).toBeInTheDocument();
  });
