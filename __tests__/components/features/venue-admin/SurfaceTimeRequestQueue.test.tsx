...
const {
  mockAnnotateSurfaceTimeRequest,
  mockDecideSurfaceTimeRequest,
  mockCancelSurfaceTimeRequest,
  mockExpireSurfaceTimeRequest,
} = vi.hoisted(() => ({
  mockAnnotateSurfaceTimeRequest: vi.fn(),
  mockDecideSurfaceTimeRequest: vi.fn(),
  mockCancelSurfaceTimeRequest: vi.fn(),
  mockExpireSurfaceTimeRequest: vi.fn(),
}));

vi.mock("@/lib/actions/venue-requests", () => ({
  annotateSurfaceTimeRequest: (...args: unknown[]) => mockAnnotateSurfaceTimeRequest(...args),
  decideSurfaceTimeRequest: (...args: unknown[]) => mockDecideSurfaceTimeRequest(...args),
  cancelSurfaceTimeRequest: (...args: unknown[]) => mockCancelSurfaceTimeRequest(...args),
  expireSurfaceTimeRequest: (...args: unknown[]) => mockExpireSurfaceTimeRequest(...args),
}));
...
describe("SurfaceTimeRequestQueue", () => {
  it("renders baseline queue details and the empty state", () => {
    ...
    expect(screen.getByText("No track time requests yet.")).toBeInTheDocument();
  });
  ...
  it("fully approves a request with the exact requested interval and space", async () => {
    ...
    expect(mockDecideSurfaceTimeRequest).toHaveBeenCalledWith(
    ...
    expect(mockDecideSurfaceTimeRequest.mock.calls[0][0].approvedStartAt).toBeInstanceOf(Date);
    expect(mockDecideSurfaceTimeRequest.mock.calls[0][0].approvedEndAt).toBeInstanceOf(Date);
  });
  ...
  it("requires an intentional venue-wide claim confirmation and reason, then sends them to the action", async () => {
    ...
    expect(mockDecideSurfaceTimeRequest).not.toHaveBeenCalled();
  ...
  it("renders server validation failures returned by decideSurfaceTimeRequest", async () => {
    mockDecideSurfaceTimeRequest.mockResolvedValueOnce({
      success: false,
  ...
  it("declines, cancels, expires, and annotates with the exact request identity", async () => {
    ...
    expect(mockCancelSurfaceTimeRequest).toHaveBeenCalledWith(
    ...
    expect(mockExpireSurfaceTimeRequest).toHaveBeenCalledWith(
      expect.objectContaining({ requestId: ACCEPTED_REQUEST_WITH_RESERVATION.id }),
    );

    await user.type(screen.getByLabelText(/internal note/i), "Flood crew confirmed split-sheet setup.");
    await user.click(screen.getByRole("button", { name: /save note/i }));
    expect(mockAnnotateSurfaceTimeRequest).toHaveBeenCalledWith(
      expect.objectContaining({
        requestId: ACCEPTED_REQUEST_WITH_RESERVATION.id,
