...
const need = {
  ...
  canClaim: false,
  assignments: [
    { id: "assign-1", status: "INVITED", personLabel: "Sam Rivera", checkedInAt: null, isSelf: false },
  ],
};
...
describe("VolunteerBoard", () => {
  ...
  it("offers close-out controls to an organizer for accepted assignments only", () => {
    const accepted = {
      ...need,
      assignments: [{ id: "assign-2", status: "ACCEPTED", personLabel: "Sam Rivera", checkedInAt: null, isSelf: false }],
    };
...
describe("RoleGrantManager", () => {
  ...
  it("does not offer roles whose work is not yet routed through grants", async () => {
    ...
    // Offered: the roles a grant actually empowers. Scheduler and event manager
    // joined the list when race rounds started checking MANAGE_SCHEDULE.
    ...
    expect(screen.getByRole("option", { name: "Scheduler" })).toBeInTheDocument();
    expect(screen.getByRole("option", { name: "Event manager" })).toBeInTheDocument();
    // Withheld: registration, finance, communications, and coaching still guard
    // on legacy roles, so a grant would do nothing.
    for (const withheld of ["Registrar", "Treasurer", "Communications lead", "Coach"]) {
      expect(screen.queryByRole("option", { name: withheld })).not.toBeInTheDocument();
    }
  });

  it("hides the race-weekend scope when the association has no rounds", async () => {
    const user = userEvent.setup();
    render(<RoleGrantManager {...props} />);
    ...
    // A scope with nothing to point at is a dead end, not a choice.
    expect(
      screen.queryByRole("option", { name: "One race weekend" }),
    ).not.toBeInTheDocument();
  });

  it("offers the race-weekend scope once rounds exist", async () => {
    const user = userEvent.setup();
    render(
      <RoleGrantManager
        {...props}
        rounds={[{ id: "clround00000000000000001", name: "Round 1 - Kari Motor Speedway" }]}
      />,
    );

    await user.click(screen.getByRole("combobox", { name: /Responsibility/i }));
    await user.click(screen.getByRole("option", { name: "Event manager" }));
    await user.click(screen.getByRole("combobox", { name: /Scope/i }));

    expect(
      screen.getByRole("option", { name: "One race weekend" }),
    ).toBeInTheDocument();
  });
