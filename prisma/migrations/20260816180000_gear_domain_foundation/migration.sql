...
BEGIN
  ...
  IF NEW."recipientEmail" IS DISTINCT FROM OLD."recipientEmail"
    ...
    IF OLD."status" NOT IN ('SENT', 'FAILED', 'CANCELED')
      OR current_setting('spartan.notification_outbox_redaction', true) <> 'on'
      OR NEW."recipientUserId" IS NOT NULL
      OR NEW."recipientEmail" <> ('redacted:' || OLD."id" || '@invalid.spartan')
...
AS $$
BEGIN
  PERFORM set_config('spartan.notification_outbox_redaction', 'on', true);
  UPDATE public."notification_outbox"
  SET "recipientUserId" = NULL,
      "recipientEmail" = 'redacted:' || "id" || '@invalid.spartan',
