...
// --- Metro Motorsport Championship fixtures (league mode, role matrix, proposal inbox) ---
...
async function seedMetroLeague() {
  const league = await prisma.league.upsert({
    ...
    create: {
      ...
      name: 'Metro Motorsport Championship',
      sport: 'MOTORSPORT',
...
async function seedLeagueVenue(leagueId: string, createdById: string) {
  return prisma.venue.upsert({
    ...
    create: {
      ...
      name: 'Metro Motor Park',
      address: '400 Paddock Drive',
      city: 'Chicago',
      state: 'IL',
      surfaceType: 'CIRCUIT',
...
async function seedProposalInbox(opts: {
  ...
  await upsertEntry({
    ...
    note: 'Saturday matchup at Metro Motor Park?',
...
async function main() {
  ...
  // Create race team
  const raceTeam = await prisma.team.upsert({
    where: { id: 'cseedraceteam00000000001' },
    update: {},
    create: {
      id: 'cseedraceteam00000000001',
      name: 'Northside Racing',
      sport: 'MOTORSPORT',
      season: 'Winter 2026-27',
    },
  })
  ...
  // Create race events
  const raceRoundDate = new Date()
  raceRoundDate.setDate(raceRoundDate.getDate() + 7)

  const raceTestDate = new Date()
  raceTestDate.setDate(raceTestDate.getDate() + 3)

  const raceRound = await prisma.event.upsert({
    ...
    create: {
      ...
      startAt: raceRoundDate,
      location: 'Madras International Circuit - Full circuit',
  ...
  await prisma.event.upsert({
    ...
    create: {
      ...
      startAt: raceTestDate,
      location: 'Madras International Circuit - Kart track',
  ...
  // Create test RSVPs for race round (self/household rows, playerId null)
  await ensureSelfRsvp(adminUser.id, raceRound.id, 'GOING')
  await ensureSelfRsvp(memberUser.id, raceRound.id, 'MAYBE')

  // Guardian fixtures (identity graph, Tier 3): parent@test.com guards the two
  // drivers without linked accounts. One per-child RSVP is answered
  // (Dylan GOING on the race round); the other (Sam) has no per-child row —
  ...
  // League-mode fixtures: Metro Motorsport Championship role matrix + proposal inbox
  ...
  console.log('   Member: member@test.com / member123 — MEMBER of Northside Racing')
  console.log('   Parent: parent@test.com / member123 — MEMBER of Northside Racing + guardian of Dylan Bouchard & Sam Kowalski')
  console.log('🏁 Motorsport: Northside Racing (Winter 2026-27) — 4 players')
  console.log('🥍 Lacrosse: Eastside Thunder (Spring 2026) — 2 players')
  console.log('📅 Events: 2 games + 2 practices scheduled')
  console.log('👨‍👧 Guardians: parent@test.com answered per child for the race round (Dylan GOING); Sam has no per-child row yet (pending)')
  console.log('')
  console.log('🏆 Metro Motorsport Championship (MOTORSPORT)')
  ...
  console.log('🏟️  Venue: Metro Motor Park (America/Chicago)')
