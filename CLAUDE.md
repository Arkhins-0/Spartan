...
Spartan is a free, open-source motorsport championship and club management platform (motorsport-only — see ADR-0012) built with Next.js 16, React 19, and TypeScript. The application uses a modern stack with MUI v7 (with Emotion), Tailwind CSS v4, Prisma 7 with PostgreSQL (Neon), and Auth.js for authentication. The project follows a mobile-first design philosophy and uses Next.js Server Actions as the primary data mutation pattern.
...
| 0011 | Model race weekends as rounds with sessions and open volunteer signup |
| 0012 | Make Spartan a motorsport-only platform |
...
## Active Technologies
- TypeScript with Next.js 16 App Router and React 19 + MUI v7/Emotion, Prisma 7, Neon PostgreSQL adapter, Auth.js v5, Zod v4, Bun (002-venue-management)
- PostgreSQL through Prisma; public logo assets require object/file storage integration selected during implementation (002-venue-management)
...
- PostgreSQL via Prisma; new `SurfaceSegment` + `SegmentCoexistence` models; `segmentId` on SeasonGame/EventGame/VenueScheduleBlock; `Venue.layout Json?`; `VenueSurface.wholeLabel`; REMOVE `IceUsage` enum + `surfaceUsage`/`zoneLabel` (SeasonGame) + `iceUsage`/`zoneLabel` (EventGame) (006-surface-segmentation)

## Recent Changes
- 002-venue-management: Added TypeScript with Next.js 16 App Router and React 19 + MUI v7/Emotion, Prisma 7, Neon PostgreSQL adapter, Auth.js v5, Zod v4, Bun
- 002-venue-management: Added venue organization onboarding, branded public circuit profiles, venue staff roles, multi-surface scheduling models, and public venue discovery contracts.
