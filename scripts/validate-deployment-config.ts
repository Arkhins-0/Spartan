...
export async function validateDeploymentConfig(rootDir = process.cwd()): Promise<string[]> {
  ...
  if (existsSync(docsBuildScriptPath)) {
    ...
    requireCondition(failures, DEFAULT_DOCS_PAGES_DOMAIN === 'dev.arkhins.com/spartan', 'Documentation build script must configure the dev.arkhins.com/spartan custom domain.');
  ...
  requireCondition(
    ...
    DEFAULT_UPTIME_TARGETS.some((target) => target.url === 'https://spartan.arkhins.com')
      && DEFAULT_UPTIME_TARGETS.some((target) => target.url === 'https://dev.arkhins.com/spartan')
      && DEFAULT_AUTH_ALLOWED_HOSTS.has('spartan.arkhins.com')
      && DEFAULT_AUTH_ALLOWED_HOSTS.has('dev.arkhins.com')
