...
export const DEFAULT_DOCS_PAGES_DOMAIN = 'docs.spartan.arkhins.com';
...
<body>
  <header><div><p class="muted" style="color:#bfdbfe;margin:0">Spartan</p><h1>${escapeHtml(page.title)}</h1><p>${escapeHtml(page.description)}</p><nav>${navLinks}</nav></div></header>
  <main>${page.html}</main>
  <footer>
    <p style="margin:0 0 .5rem">Generated from the Spartan repository docs. Main application: <a href="https://spartan.arkhins.com">spartan.arkhins.com</a>.</p>
    <p style="margin:0">&copy; 2026 Arkhins. This documentation is licensed under <a href="https://creativecommons.org/licenses/by/4.0/">CC BY 4.0</a>; Spartan source code is licensed under <a href="https://github.com/Arkhins-0/Spartan/blob/main/LICENSE">Apache 2.0</a>. &quot;Spartan&quot; and the Spartan logo are <a href="https://github.com/Arkhins-0/Spartan/blob/main/TRADEMARKS.md">trademarks</a> of Arkhins. Spartan is a fork of OpenLeague (&copy; 2025 Mark Beacom) and is not affiliated with it. Contact: <a href="mailto:tech@ctrsports.in">tech@ctrsports.in</a>.</p>
  </footer>
</body>
</html>
`;
...
async function buildAppDocsPages(): Promise<StaticPage[]> {
  ...
  for (const item of docsItems) {
    ...
    pages.push({
      title: `${item.title} - Spartan Documentation`,
...
async function buildRepositoryDocPages(): Promise<StaticPage[]> {
  ...
  for (const entry of entries) {
    ...
    pages.push({
      title: `${title} - Spartan Documentation`,
