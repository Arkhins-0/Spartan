...
How to stand up an Spartan instance: designate the platform administrator,
...
- **Venues** — add your circuits/paddocks; venues carry track layouts and schedule blocks
...
- **Pooled stock** — interchangeable quantities (tyre blankets, radio headsets). You
...
headsets go" has an answer. Review it under **Recent inventory activity**.
...
```bash
docker run -d --name spartan-dev-db \
  -e POSTGRES_PASSWORD=devpass -e POSTGRES_USER=devuser -e POSTGRES_DB=spartan \
  -p 55433:5432 postgres:16-alpine

export DATABASE_URL="postgresql://devuser:devpass@localhost:55433/spartan"
